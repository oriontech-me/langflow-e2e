...
| **Unexpected pass** — a `test.fail()` body passed (`error_signature: "expected to fail but passed"`, #2009) | any | **Possible fix day**, not a failure. The triage dataset lists it under `declared_fix_candidates[]`, never under `hard_failures[]` (#2027). It is `unexpected`, so the workflow still auto-removed `@stable`. **Never quarantine it and never file it as a failure**: confirm the upstream fix for the version under test, remove `test.fail()`, restore `@stable`, and close the issue the declaration cites. `passes.dates` are the days it passed again. |
