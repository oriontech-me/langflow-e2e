...
> The **External dependencies** section lists files from the upstream Langflow repository that, if changed, could break the test. It is parsed by `scripts/impacted-tests.ts` (`adaptive-impacted.yml` — itself `disabled_manually` today, see below), which maps a changed upstream path to the specs whose docs name it — by prefix, so a path that resolves to nothing makes the spec unselectable the day that lane is revived. It is **not** what `file-watcher.yml` reads: the watcher's coverage is the hand-maintained area table in `scripts/watch-upstream-areas.mjs`, so adding a path here does **not** make the watcher see it. Every path here is resolved against upstream on each PR by the **Spec-doc dependency paths** job (#1298), so fill it in carefully. **Write the full `src/…` path, never an abbreviation** — `api/v2/hitl.py` is not a path that fails to resolve, it is a path the resolver is never handed, so it is invisible to that job and to `impacted-tests.ts` alike, and it can be ambiguous besides (`session-selector.tsx` named two real files). The same job's second step (`--mode=check-doc-abbrevs`, #1592) catches it: a token that resolves uniquely against the Langflow tree and to nothing in this repo is reported, and fails when the diff touched that doc. When the file is named to say the spec does **not** depend on it, declare it in `scripts/lib/doc-dep-abbreviation-declarations.json` rather than writing a sentence in the doc about why it is omitted — that in-doc form is the defect #1587 removed.
...
The area table is **not duplicated here** — it lives in `scripts/watch-upstream-areas.mjs` (14 areas → paths, tags, checklist sections, and the commands to run) and is printed by:
...
>
> **What no signature can reach, and the second exemption that does (#1763).** The paragraph below explains why the list stays narrow, and that narrowness has a cost the two paragraphs above cannot pay off: an exemption keyed on the error TEXT only ever sees a failure that *reports* the transport. A spec that wraps its wait in an assertion reports the **state that never arrived**, so a wedge-caused failure of it classifies `null` on every attempt of every run — and `actionable = recurrent && !infra_signature` then resolves to `true` on a shard measured 41 % down. **Widening the list cannot fix it**: the message is about the state, not about the transport, and every pattern that would match it (`expect(...)`, `waitForSelector: Timeout`) is on the excluded list for the reason below. The answer is a different KIND of evidence — the run's own per-attempt liveness overlap, recorded on the history row as `outage_overlap` — and it is scoped to the **flake** path only: that decision is a human-confirmed proposal, where `@stable` auto-removal is an unreviewed commit, so the overlap is allowed to demote a flake and is deliberately **not** wired into the removal above. Rules, thresholds and the measured instances: *Weekly triage protocol* → **2. Flakes**.
>
...
- **Recurrence window: 30 days.** The criterion is the **cause, not the count**: it is not enough that the test flaked 2+ times in the window — the occurrences must share the **same cause**, which the triage dataset decides by comparing each row's `recurrence_keys` (#1626) — the failure's kind, the element or request it waited on, the file it failed in and that line's source text, for **every** failed attempt — rather than `error_signature` alone, which named neither the element nor the call site (one spec collided with itself, #1623/#1694) and carried the model and counters a marker assertion interpolates (one cause never matched itself, #1665/#1679). A row written before the keys can only be compared on the head, so its date is still counted but listed in `recurrence.unverified_dates`: confirm it against that run's call log before citing the figure. Same cause within the window → recurrent → open a **dedicated issue and quarantine the test via PR as prevention** (flake quarantine is always manual; the workflow never auto-removes flakes) — so the flaky test stops running until it is worked. **Quarantine = remove `@stable` AND wrap the test as `test.fixme(<reason + issue #>)`, together**: `@stable` removal alone only stops the daily, so the test keeps going red on the `pr-validation.yml` impacted-specs gate (which selects specs by file diff, not by tag — the quarantine PR itself would go red; #871). `test.fixme` skips it in every context so the quarantine PR merges green. Lifting the quarantine (remove `test.fixme` + restore `@stable`) is a **deliverable of that issue**, done after the fix. *(30 days is a revisable convention, not an absolute.)*
...
- **The signature is blind to an assertion-shaped failure, and the measurement answers for those (#1763).** `infra_signature` matches the error TEXT against a transport-level list, so it can only ever see a failure that *reports* the transport. A spec that wraps its wait in an assertion reports the **state that never arrived** — `expect(received).toBe(expected)`, `"de-AT": the application never reached its main page` — which is most specs, and the wedge that caused it leaves no string to match on any attempt of any run. **This is not a gap in the pattern list and widening the list cannot close it.** So a second exemption reads a *measurement* instead: the run's own liveness recorder already knows, per shard, when the backend answered nothing, and the merged report carries every attempt's span. The appender records the join on the row as `outage_overlap` (`reports/README.md` → *Field semantics*), and the triage dataset demotes a recurrent flake to `actionable: false` with an **`outage_excluded`** block when **every** failed attempt of it sat at least `OUTAGE_COVERAGE_THRESHOLD` (0.5) inside a measured outage on its own shard. Four things to know before reading one. **The fraction is the instrument, not the overlap** — a boolean "it touched a window" is close to a coin flip on a shard measured 33–73 % down (`collateral_attempts > 0` on 11 of the 15 days carrying a `backend` block), while the two measured instances scored 0.82/0.87 (`agent-system-prompt.spec.ts:213`, 2026-09-08) and 0.66 (`locale-resilience.spec.ts:116`, 2026-09-10) against 0.046 for the 6-second blip the issue names as the case that must **not** exempt anything; read `min_coverage` against the `shard_down_pct` printed beside it. **It is `min`, not `max`** — one corroborated attempt beside one clean one is a test that failed while the backend was answering, which is the spec's own failure. **The signature keeps precedence**: an `ECONNREFUSED` is transport-level whatever the backend was doing, so `infra_excluded` renders and `outage_excluded` does not. And **`unmeasured`, `clear` and an absent block all leave the flake actionable** — absence of evidence exempts nothing (#1012), which also means `weekly-stable.yml`, with no liveness recorder, is unaffected. Below the threshold the flake stays actionable and **still carries its `outage_overlap`**, so the proposal prints the downtime it sat in rather than making the analyst rediscover it: both instances above were declined by hand, each after downloading three runs' `results.json`, four `liveness-N` artifacts and four container logs.
- **A past occurrence's backend state is on the row too.** `recurrence.outage_by_date` names, per same-signature date in the window, whether that run measured the occurrence as `overlapped`, `clear`, `unmeasured` or `unrecorded` (a row written before #1763). The `liveness-*` artifacts expire after 7 days against a 30-day window, so without it a later triage recomputing recurrence sees the same `actionable: true` with no trace of a refutation already paid for.
...
| **Unexpected pass** — a `test.fail()` body passed (`error_signature: "expected to fail but passed"`, #2009) | any | **Possible fix day**, not a failure. The triage dataset lists it under `declared_fix_candidates[]`, never under `hard_failures[]` (#2027). It is `unexpected`, so the workflow treats it as a hard failure and may have auto-removed `@stable` (not on a guard day, and not when an earlier failed attempt carries an infra signature). **Never quarantine it and never file it as a failure.** A fix is confirmed only by **both** a pass on at least two dailies (`passes.count >= 2`, counted per provider variant) **and** the upstream fix located in the image under test by its version — a fix on a release branch is not yet in the nightly. The triage enriches the issue the `test.fail()` cites with that evidence; removing `test.fail()`, restoring `@stable` and closing the bug are that issue's deliverables. |
