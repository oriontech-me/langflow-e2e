...
- **`file-watcher.yml`** — Detects upstream Langflow changes in monitored paths and opens a GitHub issue with the exact `--grep` command needed to revalidate affected areas. **`workflow_dispatch` only** (the schedule was disabled in `9da85fa`), with a `since` input so a dispatch can cover a longer gap than 24h. The 13-area table, the `src/lfx/` decision record and both guards live in `scripts/watch-upstream-areas.mjs` (#1092, covered by `npm run test:scripts`); the workflow only wires them. Two omissions used to make a clean verdict worthless, and both are now **fail-closed**: `src/lfx/` — the package Langflow keeps moving backend behavior into — was watched by **zero** of the 13 areas, which is how the change that broke all six stdio registrations in `mcp-server.spec.ts` (#1091, `lfx/base/mcp/security.py`) never fired the watcher; and a monitored path that no longer existed was **silent**, because `git log --since -- <gone-path>` prints nothing, so "this path is dead" and "nothing changed here" produced the same empty string (`constants/flow_constants.tsx` had already moved). `--mode=check` now runs before the sweep and fails naming the path, and **every** `lfx` subtree must be classified — mapped to an area or recorded out of scope with a reason — so the next step of the `lfx` migration forces a decision instead of widening the blind spot. Upstream commit subjects reach `github-script` through the environment, never interpolated into the script body.
