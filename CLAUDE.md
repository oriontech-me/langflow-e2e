...
- **`tests/fixtures/fixtures.ts`** — Always import `test` from here, never directly from Playwright. It extends the base `test` with backend HTTP error monitoring and flow execution error detection. **The two are not equally strong, and the difference decides how much a green run is worth (#1084):** a `flow_error` **fails** the test; an `http_error` is **logged and never fails it**, on any path, for any endpoint — so the only thing between a real backend 500 and a green test is a human reading the log (checklist step 4 below, `CONTRIBUTING.md` step 5). Which responses reach that log is decided by `tests/fixtures/http-error-policy.ts` — every 4xx/5xx on an `/api/` route except the documented exemptions (auth endpoints; the external Langflow Store, unreachable in CI). It genuinely covers 4xx/5xx now; before #1084 it matched four exact codes and silently missed 401/403/405/409/502/503, which is why `execution-error-notification` could mock a 503 specifically to slip past it. Escape hatches: `page.allowFlowErrors()` for tests that provoke execution failures, `page.allowHttpErrors()` for tests that drive an endpoint into a 4xx/5xx on purpose (keeps the advisory log trustworthy). `PW_HTTP_ERROR_DEBUG=1` prints what the policy ignored, with reasons. **Which run-stream responses count as a `flow_error` is decided by `tests/fixtures/flow-error-policy.ts`** (#1162, covered by `npm run test:units`), and on 1.12.x that half was **dead for every playground/agent run** until it was fixed: the run is `POST /api/v2/workflows`, which the inline URL list (`/build/`, `/run/`, `/events?event_delivery=`) did not match, served as `text/event-stream`, which the inline detector skipped wholesale — so an Anthropic 400 reached the UI with no verdict at all, and `page.allowFlowErrors()` was silently a no-op for the 17 specs that call it. The policy now matches the v2 shapes (`type: "RUN_ERROR"`, `event_type: "error"`, a node `status: "error"` carrying `errorMessage`) alongside the v1 ones, parses SSE and NDJSON alike, and reads an SSE body with a budget that can actually outlast a run — a stream closes only when the run does, so the old 2 s bound was a guaranteed miss. Because such a read lands late by construction, the fixture **waits (bounded) for in-flight run-stream reads before rendering its teardown verdict**, and says out loud when a read did not finish — an unevaluated stream is unknown, not clean (#1012's rule). Note the trap that hid all of this: the error event's message object carries `"error": false`, so the old `data.error === true` check read a failed run as healthy.
