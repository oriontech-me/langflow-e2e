...
- **A local instance records nothing, and developer spend is OUT OF SCOPE for this series
  ...
  secret and a developer's `.env` share one account balance (#1169) and #1183's key-separation
...
- **`pr-validation` and `manual` measure but deliberately do not write history.** Both run the
  ...
  at all: 12 consecutive runs recorded `0 trace(s)`, including one that executed 20 `llm-agents`
  specs against a configured provider (run 31018914069).
