...
- `listing_completeness` (optional, additive to schema v1, #1812/#1818) is the **other half** of the field above, and the two fail apart. `collection_gate_keys` says which suite the listing *environment* would produce; this says whether the listing then produced it, by diffing the `--list` against every spec file that declares an `@stable` test on disk. Two lanes can resolve identical keys and one of them still lose a file — a listing that dies halfway and exits 0, a future spec gating collection on something that is not a provider key — and the loss leaves **no failure, no skip and no row to be missing from**, only a smaller total.
  ...
  - **Both lanes fail their run on an unverified check; only one of them always fails on a non-empty `missing`.** The Actions lane does, always — it is fully keyed, so nothing was accepted. On the VM, severity follows the collection-gate plan: a run that set `REQUIRE_PROVIDER_KEYS=0` was explicitly told to *accept the narrower comparison*, and a spec generated entirely from an absent key is what that costs, so it is reported and recorded rather than reddened. Measured: with all three collection-gating keys blank a clone lists 246 of 247 files, and the missing one is exactly the file those keys generate.
