...
- `report_missing` (optional, additive to schema v1, #1176) marks a line written with **no Playwright report at all**. When every shard aborts before its first test no blob exists, the merge step fails, and `results.json` is never written — the appender used to `exit 0` there, so the step went green and the day simply **vanished** from this file (2026-07-31 is absent for exactly this reason: a `jq` query reads 07-30 followed by 08-03 as consecutive weekdays). Such a line carries zero `totals` and is selected by the same *runs that executed NO test at all* query as any other infra abort; the field is what separates **there was no report** from **the report reported errors**, which are different diagnoses. Its `run_errors[0]` is **synthesized by the appender** rather than read from a report — the only place that happens — so the published query prints a cause instead of its `no recorded reason` fallback. Written only in CI: run locally with no report the appender still writes nothing, because this file is machine-owned and committed. Absent from history written before #1176, and absent on every ordinary day.
