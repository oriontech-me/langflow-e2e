...
| `start-ollama-source.sh` / `stop-ollama-source.sh` | Ollama WITHOUT a container, for the same VMs. Runs the pinned release (the version the `ollama-e2e` image carries — `docker/ollama-e2e/Dockerfile` pins the base tag and a unit test reads BOTH the version and the model out of it, so the container and this script cannot drift apart), verified against the release's `sha256sum.txt`. Refuses loopback and any non-RFC-1918 address, because Langflow's SSRF layer decides whether the provider call the spec asserts on is allowed at all. **Ready means the MODEL is present**, not just the server: a model-less instance answers `/api/tags` perfectly and makes `ollama-provider.spec.ts` SKIP, which is a green lane one test short — so the script pulls what is missing and fails if it is still absent. Prints `OLLAMA_HOST_IP` / `OLLAMA_PORT` / `OLLAMA_MODEL`; unlike the echo it needs no resolver, because the spec already reads `OLLAMA_BASE_URL`, `OLLAMA_BASE_URL_FROM_LANGFLOW` and `OLLAMA_TEST_MODEL` and only their defaults are container-shaped |
...
| `ci-change-coverage.mjs` | Decides what a CI-only diff gets on a PR: `canary` (the PR lane runs what changed → fixed 3-spec set), `dispatch` (another lane's surface → name it), `none`. Reachability derived from the YAML, not hardcoded (`#1159`). `dispatch` also renders the advice, because naming a workflow is not the same as naming a remedy: it confirms the workflow **can** be dispatched — trigger read from the **default branch**, which is where GitHub resolves `workflow_dispatch` from, plus the Actions state listing (four of ours are `disabled_manually`) — and words six cases, splitting a mixed diff rather than flipping it. Never in the YAML: composed there with `jq`, it prescribed a 422 (`#1609`). Reachability follows `uses:` AND imports — a shared module is spelled in no workflow, so all 24 of the `scripts/lib/**` files it now reaches used to resolve to `none` (19 of them reachable only by import); `canary` and `dispatch` are a union, not a race; a unit test is never CI wiring whatever a comment spells (`#1979`) |
