...
test.describe("MCP Server — per-project tool exposure", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    // first version of this spec used (`e2e_mcp_project_config`) left nothing
    // past the first three digits, so every project it created collided with the
    // previous one: `POST /api/v1/projects/` → 409 "MCP server name conflict:
    ...
    // that version failed. The helper now budgets the discriminator first and
