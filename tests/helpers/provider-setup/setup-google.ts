...
export async function setupGoogle(
  ...
  // What it must NOT do is click every toggle: 29 enable updates in one request is
  // ~93 s of a blocked single worker, ending in gunicorn's SIGKILL with nothing
  // persisted, and it is the model picker's own defaults that already carry what
  // this setup goes on to select (#1679 — measured in model-toggle-batch.ts).
