...
- [x] Agent returns output in correctly rendered Markdown → `llm-agents/agent-markdown-output.spec.ts` (**`@stable` since #1790**, Wave 8 T1. Measured, not assumed: three `manual.yml` dispatches at `retries=0` / `provider=auto` on the nightly, green on openai, anthropic and google in every pass (9/9). The `MODEL_TOGGLE_WRITE_STALLED` row the triage table carried is stale — its mechanism, `setupGoogle` clicking all 29 provider-panel toggles, was replaced by the scoped write in `53f49475` (#1679) 13 h after that table merged — and one dispatch is the positive control, running the Google variant on the COLD enable path and passing)
