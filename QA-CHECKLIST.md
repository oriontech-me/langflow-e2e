...
- [x] Templates — navigation to the template selection tab (the New Flow gallery: tabs, featured cards, search) → covered by `core-functionality/templates/templates-gallery.spec.ts`, `flow-functionality/create-flow-from-template.spec.ts` (#820, #1863)
...
> The pilot of the **API coverage gauge** (#1692). `/openapi.json` is not the API: measured on `1.13.0.dev0`/`dev1`, the document carries 120 operations while the instance's router table carries **254**, 137 of them hidden — including `login`, `api_key`, `variables` and `custom_component`, three of which existing specs already drive. The denominator is therefore the router table, committed as `tests/assets/api/api-surface-baseline.json`, and a spec **declares** the operations it covers while the fixture **verifies** the declaration against what it actually issued. `npm run api:coverage` reads `15/15` for this family and `15/204` overall. Mechanism, exclusions and the traps: `docs/api/api-surface-coverage-gauge.md`; spec docs `docs/api/files/api-files-v1-flow-scoped.md` and `docs/api/files/api-files-v2-store.md`.

- [x] v1 flow-scoped store round-trip: `POST /api/v1/files/upload/{flow_id}` answers `201` with `file_path` = `<flow_id>/<timestamp>_<name>`, `GET .../list/{flow_id}` lists exactly that stamped name, and `GET .../download/{flow_id}/{file_name}` returns `application/octet-stream` **byte-identical** to the asset — the bytes, not the length, since a truncating proxy passes a length check → `api/files/api-files-v1-flow-scoped.spec.ts`
- [x] v1 delete: `DELETE /api/v1/files/delete/{flow_id}/{file_name}` answers `200` naming the file, the listing drops to `{"files": []}` and the download no longer resolves → `api/files/api-files-v1-flow-scoped.spec.ts`
- [x] v1 `images/` serves a real PNG as `image/png` (the content type is the assertion, not the `200` — this endpoint backs the chat image preview), and uploading to a flow that does not exist is refused `404 "Flow not found"`, which is the premise assertion for the whole family → `api/files/api-files-v1-flow-scoped.spec.ts`
- [x] v1 bundled profile pictures: the list is non-empty and every entry is `<folder>/<name>.svg`; fetching the first returns `image/svg+xml` → `api/files/api-files-v1-flow-scoped.spec.ts`
- [x] v2 user store round-trip: `POST /api/v2/files` answers `201` with the display name **stripped of its extension**, `path` = `<user_id>/<filename>` and `provider: null`; the listing agrees; `GET /api/v2/files/{file_id}` returns the **content** (not metadata); and `DELETE /api/v2/files/{file_id}` confirms under **`detail`** with the id gone from the listing — asserted rather than left to the cleanup hook, which resolves on any status → `api/files/api-files-v2-store.spec.ts`
- [x] v2 rename is metadata-only: `PUT /api/v2/files/{file_id}?name=` answers `200` with the new name and the **same** `path`, the listing agrees, and the bytes are unchanged → `api/files/api-files-v2-store.spec.ts`
- [x] v2 batch: `POST /api/v2/files/batch/` answers `application/x-zip-compressed` whose body carries the ZIP magic and both members named `<reported name>.<extension>` (the store suffixes duplicate display names, so asserting the asset filenames would be order-dependent), and `DELETE /api/v2/files/batch/` answers `200` under **`message`** — the key differs from the single delete's `detail`, so both are asserted by key → `api/files/api-files-v2-store.spec.ts`
- [x] v2 batch requires its trailing slash: without it `POST` answers `405` and `DELETE` answers `422` with `type: "uuid_parsing"`, `loc: ["path","file_id"]` — the path location is the evidence the request reached the *single-file* route (`input: "batch"` stopped being echoed in `1.13.0.dev10`, upstream #15038 / LE-2462, #1841) — **and the file is still listed**, which is the load-bearing half: a refusal that had nonetheless deleted it is the actual hazard → `api/files/api-files-v2-store.spec.ts`
- [-] `DELETE /api/v2/files` empties the caller's whole store, asserted as `[]` and not as a delta (the contract is "everything"; a delta passes against an endpoint that deleted only this test's two uploads). **`@destructive`**, deliberately not `@stable`: the store is per-user and every worker shares the superuser, so there is no scheduled lane for it (#1010) → `api/files/api-files-v2-store.spec.ts`
- [~] `GET /api/v1/files/images/{flow_id}/{file_name}` answers **`500`** `"Content type text/plain is not an image"` for a non-image — a client error served as a server error. Asserted from the positive side only and recorded here: pinning the `500` would make the suite defend the defect, and `422`-versus-`500` is a product choice this repo should not make by assertion.

#### 1.11 Flows API — the whole family, as a contract (1.13)

> Second family of the API coverage gauge (#1699, after `files` in § 1.10): `/api/v1/flows`, **21 operations**, all measured on `1.13.0.dev0` before being asserted. Eight were already driven as contracts by `api-flows-crud.spec.ts` and counted for nothing because they did not declare — they now do, with no assertion changed. Adopting `api-flows-batch.spec.ts` found it asserting a bulk-DELETE endpoint that does not exist (`POST /flows/batch` → `405`; the route is batch **create**), red on the nightly and invisible to the daily. `npm run api:coverage` reads `/api/v1/flows 21/21`. Spec docs under `docs/api/flows/`.

- [x] `PUT /api/v1/flows/{id}` is a **merge, not a replace**: `name` is the one required field (`422` naming `body.name` without it), and a body carrying only `name` keeps `description`, `folder_id` and every omitted field — asserted in both directions, plus a `data` replace that still preserves `description` → `api/flows/api-flows-put-and-bulk-delete.spec.ts`
...
- [x] `GET /api/v1/flows/basic_examples/` (`Accept-Language: en-US`) lists the starter flows with `name`, `description` and `data.nodes`, `Simple Agent` among them → `api/flows/api-flows-public-and-metadata.spec.ts`
...
- [x] Two projects whose names share their first 26 characters are both created (`201`, was `409 MCP server name conflict` before Nightly 1.13.0.dev21, #1409), each with its own `lf-…` MCP server entry, matched by the project id in the entry's `args` → `api/projects/api-projects-crud.spec.ts`
- [x] `GET /api/v1/projects/download/{id}` refuses an **empty** project (`404 "No flows found in project"`) and returns a **ZIP** (asserted by magic bytes, which is stronger than the `application/x-zip-compressed` header the route does send — an earlier note here said it sent none) once it holds a flow; the populated-project read carries a `detail`+flow-readback diagnosis after `LE-2598` made it answer `404` for a flow whose `INSERT` had not committed (#1807, fixed in `1.13.0.dev14`) → `api/projects/api-projects-transfer.spec.ts`
...
- [x] A `422` **never echoes the submitted value** (LE-2462 / langflow-ai/langflow#15038, the contract #1841 caught by accident and #1845 deliberately left uncovered): a per-run sentinel is absent from the **raw body** of five shapes — a `missing` sibling (where FastAPI's default echoed the whole enclosing object, credential included), a validator that quotes its input (`icon` → the literal `[redacted]`, the positive control that the scrub ran), a non-uuid path param, and two schema-`ctx` cases. `ctx` is **dropped** on `uuid_parsing`/`value_error` and **kept** on `string_too_long`/`string_pattern_mismatch` — the pair that a "strip everything" handler fails; gated on `1.13.0.dev10`, the first build carrying the handler → `api/instance/api-validation-redaction.spec.ts`
...
- [x] Dropdown refresh button repopulates the option list — the refresh row is clicked, the list shows its loading state, returns to "Select an option", and the flow the test created is then selected out of that list. Both halves were force-failed rather than assumed: removing the refresh click alone times out the `text=Loading` wait at 30 s, and stripping the target flow from the `POST /api/v1/custom_component/update` response the option list is built from fails the option selection at 20 s. (The second mutation also fails with the refresh click removed, so the option delivery is that endpoint's rather than the click's — the click is what the loading-state assertion covers.) → `flow-functionality/run-flow.spec.ts` (test *user should be able to use Run Flow without any issues*, `@stable`). This bullet replaces `ui-ux/refresh-dropdown-list.spec.ts`, **deleted 2026-09-18 (#1913)**: that spec's only closing line was `getByText("Loading Options").isVisible()` — a boolean nobody read — so it could not fail. Proven rather than argued: with the string replaced by one that can never exist it still passed, 11.8 s. Its `skipped in 3/3 run(s)` verdict was a drained Anthropic key at measurement time (`collect-models` recorded *credit balance is too low* on run 34402231653), not a product fact
...
- [ ] ~~Composio (tool integration for Agent)~~ — **out of scope (2026-08-06):** Composio is a separately-shipped vendor bundle, and this QA team no longer supports that surface. `composio.spec.ts` exists but must not be promoted: doing so would count coverage the team does not sustain. See `docs/coverage-heatmap/` — vendor bundles are the single largest source of upstream defects (227 issues) and are excluded from the risk model for the same reason. **Parked 2026-09-18 (#1913 → #1916):** the spec now carries the `probeProviderComponent()` gate `docs/component-distribution-policy.md` prescribes for a distribution the image does not install — the same treatment as Groq/Mistral, and one that re-opens by itself if `lfx-bundles` is ever installed — plus an owner and a doc (`docs/core-functionality/llm-agents/composio.md`). Measured on `1.13.0.dev15`, the decision is no longer only a policy call — `GET /api/v1/all` returns 29 categories / 183 component types with **zero** `composio` hits (the 30th top-level key is `component_display_names`, a metadata map and not a category — counting it is what doubles the type total), `src/lfx/src/lfx/components/composio/__init__.py` is an `lfx-bundles-shim` and `import lfx_bundles` raises `ModuleNotFoundError` in the container, and `COMPOSIO_API_KEY` is **not a repository secret**, so the spec's own gate has never opened in any lane since it was imported
- [ ] ~~Composio Tools places on the canvas without a premature `api_key` error~~ — **out of scope**, same 2026-08-06 decision as the bullet above. **Parked 2026-09-24 (#1912):** the second half of the T2 packaging cluster, carrying ComposIO's absence from the canvas side where `composio.spec.ts` carries it from the component side. Measured on `1.13.0.dev22`: zero `composio` component entries in `GET /api/v1/all` — the four raw substring hits in the body are the `replacement = ["composio.ComposioGmailAPIComponent"]` attribute of Google's own legacy Gmail loader, pointing at a class the image does not ship — and the `lfx-bundles-shim` plus `ModuleNotFoundError` make it packaging rather than drift. Given the same `probeProviderComponent()` gate, which re-opens by itself. Doc: `docs/flow-functionality/generalBugs-shard-11.md` → `flow-functionality/generalBugs-shard-11.spec.ts`
...
- [-] Agent stops when configured stop condition is reached → `core-functionality/llm-agents/agent-max-iterations.spec.ts` (`max_iterations` is the Agent's only configurable stop mechanism — no dedicated stop-condition field exists; see #824). **Quarantined 2026-09-23 (#1991):** "agent stops when max iterations is reached" is `test.fixme` without `@stable` for a confirmed product regression, [LE-2728](https://datastax.jira.com/browse/LE-2728). The cap fires, but the limit message is lost whenever the model writes text alongside its tool call. The causal control keeps `@stable`. Lifting the quarantine is a deliverable of #1991.
- [-] Agent stops when maximum number of iterations is reached → `core-functionality/llm-agents/agent-max-iterations.spec.ts` (**#1264 was a test defect, and its second pass corrects the first:** the cap IS enforced on 1.12.0.dev39 — one bubble reading `Model call limits exceeded: run limit (1/1)`, wording unchanged. The blamed cause was NOT the SSRF-blocked target: the test recurred on 2026-08-13 with that fix merged and `ECHO_BASE_URL` pointing at a reachable go-httpbin. `ModelCallLimitMiddleware.before_model` can only fire on the SECOND model call, reachable only through the tools node, so the assertion depends on the model electing to emit a `tool_use` — `claude-haiku-4-5` answered in prose (`calls: 1`) and no limit message was produced. Test 1 now asserts that tool step separately, so non-compliance no longer reads as a broken cap. Both halves `@stable`; anthropic is unmeasurable while its key is out of credit — superseded for Test 1 by the quarantine that follows). **Quarantined 2026-09-23 (#1991):** "agent stops when max iterations is reached" is `test.fixme` without `@stable` for a confirmed product regression, [LE-2728](https://datastax.jira.com/browse/LE-2728). The cap fires, but the limit message is lost whenever the model writes text alongside its tool call. The causal control keeps `@stable`. Lifting the quarantine is a deliverable of #1991.
...
- [x] Web Search component places on the canvas, offers its three search modes (Web / News / RSS) and persists its configured query — asserted from server truth (`GET /api/v1/flows/{id}`): one `UnifiedWebSearch` node, the typed sentinel in `template.query.value`, `search_mode` defaulting to `Web` with all three options, and a single `results` output carrying the `Table` type → `core-functionality/llm-agents/web-search-component.spec.ts` (**replaces `duckduckgo.spec.ts`, #1912.** The DuckDuckGo Search component is on no tested image, and — unlike the parked specs in that cluster — not through `lfx-bundles` packaging: there is no `lfx/components/duckduckgo` directory at all, so no distribution exists for a gate to wait for. The capability moved into the core `UnifiedWebSearch`, whose `Web` mode *is* the DuckDuckGo search. No live search is executed: the deleted version raced `built successfully` against `ratelimit` and its ratelimit branch passed when the search did **not** work, on top of calling the public internet — the coupling #1128 removed. What is given up is stated rather than hidden: this proves the search component ships, places, configures and wires, never that a query returns results)
- [-] YouTube Transcripts component runs and returns a transcript → `core-functionality/llm-agents/youtube-transcripts.spec.ts` (**parked 2026-09-24 (#1912)** on the same `lfx-bundles` packaging decision as Groq/Mistral in § 7.6 — `src/lfx/src/lfx/components/youtube/__init__.py` is an `lfx-bundles-shim`, `import lfx_bundles` raises `ModuleNotFoundError` in the container, and on `1.13.0.dev22` `GET /api/v1/all` carries zero `youtube` entries across 32 categories / 200 component types. The bare `test.skip` it was imported with — inert and unattributed — is replaced by the `probeProviderComponent()` gate the standing policy prescribes, which re-opens by itself if the image ever installs `lfx-bundles`. Doc: `docs/core-functionality/llm-agents/youtube-transcripts.md`. Known and left standing, as on the Groq/Mistral bullets: `[-]` also feeds the generated *Needs validation* column, so the counts read this as scheduled work although the surface is in no image)
...
- [x] Agent returns output in correctly rendered Markdown → `llm-agents/agent-markdown-output.spec.ts` (**`@stable` since #1790**, Wave 8 T1 — with its assertion fixed first. Green 9/9 on 2026-09-14, then on 2026-09-15 one red in eight: `claude-haiku-4-5` skipped the bold run and the test failed on `strong` with nothing wrong in the renderer, and the file-level serial mode skipped `[google]` behind it. The four-construct requirement depended on the model obeying the whole prompt (#1187), so the expectation is now derived from the reply the run persisted (`GET /api/v1/monitor/messages?flow_id=…&sender=Machine`) and each provider block is serial on its own; re-measured 9/9 on the change (runs 35003644385 / 35003647350 / 35003650561) with four force-failures observed red. The `MODEL_TOGGLE_WRITE_STALLED` row the triage table carried predates #1679's scoped write, merged 18 h after that table, and did not reappear in any dispatch)
...
- [x] A math prompt typed in the Playground reaches the Agent run exactly once — the user message reads `2+2` (never `2+22+2`) and the answer is `4`, never `26` → `core-functionality/llm-agents/general-bugs-agent-sum-duplicate-message-playground.spec.ts` (**`@stable` since #1790**, Wave 8 T1 — the promotion #1465 deferred. The Wave 8 measurement could not judge it at all: it skipped 3/3 on a drained anthropic credential. Re-measured once the credential was live — three `manual.yml` dispatches at `retries=0`, each confirming `✅ anthropic (claude-haiku-4-5)` before the run, green 3/3. `@components` and `@workspace` were dropped with the promotion: both are cross-cutting, so the old array named no product area at all and put an anthropic-dependent agent spec inside `--grep @workspace`. The teardown now deletes the id `load()` returns instead of parsing the canvas URL — re-measured 3/3 on that change, and both assertions force-failed on CI from throwaway branches)
...
- [x] Configure and execute flow with Ollama (local model) → `model-provider/ollama-provider.spec.ts` (both halves `@stable`; the execution half lost the tag twice and got it back twice, each time on CI evidence rather than a local green. After #931: the `lfx-ollama` packaging gap that broke it on 07-23/24 is fixed upstream, a build-side bundle pre-flight fails attributed instead of timing out blind for 30 s, the run-completion wait was rebuilt on the `button-stop`/`button-send` signal, and the test model is derived from the instance instead of a hardcoded tag that could skip silently. After #1302: the node was losing its configuration between the model selection and the run — `Model Name` empty, base URL back to the component default — so no run could start and it surfaced 180 s later as a missing chat message; `node-config-guard.ts` now re-reads the widget immediately before `button-send`, and the three sidebar adds go through the repaired helpers. Restored on 40 `manual.yml` dispatches at `-f retries=0` on `1.13.0.dev5`, the last 20 of them 20/20 green)
- [-] Configure and execute flow with Groq → `model-provider/groq-provider.spec.ts` (**parked — a terminal state, not a pending validation.** The Groq component ships in the `lfx-bundles` distribution, which `langflowai/langflow-nightly:latest` does not install, so the spec's availability pre-flight skips it on every run — re-confirmed on `1.13.0.dev8`/`1.13.0.dev12`, where `GET /api/v1/all` carries no `groq` at all. Component availability is a packaging decision per image and the standing answer is `docs/component-distribution-policy.md`, whose policy table prescribes exactly this `[-]` marker plus the stated reason; `@stable` stays off until the component returns to the image. Known and left standing: `[-]` also feeds the generated *Needs validation* column and the Phase 1 *Validate* table, so the counts still read these two as scheduled work — a terminal marker would take a legend, a policy and a `coverage-summary.ts` change, which is its own issue rather than this one. The API key is **not** a factor — `GROQ_API_KEY` has been a repository secret since 2026-07-09 and reaches the run step on the daily and PR lanes, and the pre-flight skips before it is read. See #1039)
- [-] Configure and execute flow with Mistral → `model-provider/mistral-provider.spec.ts` (**parked** on the same `lfx-bundles` packaging decision as Groq above, with the same marker rationale and the same standing answer in `docs/component-distribution-policy.md` — re-confirmed on `1.13.0.dev8`/`1.13.0.dev12`, where every `mistral` occurrence belongs to some other vendor's component (Bedrock model ids, the Astra Vectorize provider list and its help text), never to a component of its own. `MISTRAL_API_KEY` has been a repository secret since 2026-07-09 too, and changes nothing while the pre-flight skips first. See #1039)

#### 7.7 Model Parameters (Agent)
- [x] Maximum token count — response truncated as configured → `llm-agents/agent-max-tokens.spec.ts`
- [-] Maximum agent iterations → `core-functionality/llm-agents/agent-max-iterations.spec.ts` (both halves of the causal pair `@stable` — superseded for Test 1 by the quarantine that follows; #1264's second pass added the tool-step precondition to Test 1 — see §6.2 for the mechanism and the spec doc's Tags section for the per-provider measurement). **Quarantined 2026-09-23 (#1991):** "agent stops when max iterations is reached" is `test.fixme` without `@stable` for a confirmed product regression, [LE-2728](https://datastax.jira.com/browse/LE-2728). The cap fires, but the limit message is lost whenever the model writes text alongside its tool call. The causal control keeps `@stable`. Lifting the quarantine is a deliverable of #1991.
...
#### 7.9 Langflow Assistant — provider gate (1.13)
- [x] A configured local Ollama reaches the Assistant on both of its surfaces: `GET /api/v1/agentic/check-config` lists an `Ollama` entry whose models lie between the instance's own completion + tools and completion tags (oracle: Ollama's `/api/show`, read from the test host), with no embedding-only tag; on a canvas the panel renders its composer, the selector's `Ollama` group equals the enabled Ollama LLMs the panel's two flow-scoped catalog reads imply, the local model arms Send once a draft exists, and a ≤13B tag carries the weak-model hint. Keyless and nothing is sent (#1810) → `core-functionality/model-provider/assistant-ollama-provider.spec.ts`
- [ ] The Assistant gate without a live provider, by route interception: the keyless `check-config` contract on an unconfigured instance, the no-models state and its CTA, and the feature gate (`agentic_experience` via `/api/v1/config`) outranking a configured provider — plus narrowing the `/api/v1/agentic/` exclusion in `tests/assets/api/api-surface-baseline.json` (#1810)
...
- [-] Voice mode (voice assistant) → `ui-ux/voice-assistant.spec.ts` — **parked 2026-09-18 (#1913 → #1915)**, and the old note ("spec is a stub") understated it: the surface is **not in the tested image**. On `1.13.0.dev15` the served bundle carries zero occurrences of `voice-button`, `voice_mode_available` or `voice-assistant`, and the open playground reports `voice-button` 0 / `audio-button` 1 with `voice_mode_available: true` mocked and the route confirmed to fire — the playground rewrite swapped `VoiceButton` for an `AudioButton` (speech-to-text). The row the #1784 measurement scored `3/3 green` is **not** a promotion candidate: it asserts `not.toBeVisible()` on an element that cannot exist under any config value — measured, not argued: it passes with the mock inverted to `true`, and it passes with the route mock deleted outright. Doc: `docs/ui-ux/voice-assistant.md`. **`audio-button`, the control that replaced it, is covered by no spec** — that is the gap this opened, tracked in #1915. Known and left standing, as on the Groq/Mistral bullets in § 7.6: `[-]` also feeds the generated *Needs validation* column and the Phase 1 *Validate* table, so the counts read this as scheduled work although all three tests are `test.fixme` and the surface is in no image — measured, this module goes `Needs validation` 0 → 1 and Phase 1 `Validate ([-])` 0 → 1 on the next regeneration. A terminal marker would take a legend, a policy and a `coverage-summary.ts` change, which is its own issue rather than this one. `[!]` (*covered but flaky*) is what it cannot stay: nothing here is flaky, it does not run
...
- [x] All templates tab — `side_nav_options_all-templates` renders exactly one `template_<slug>` card per registered template, the expected set read from the listing rather than hardcoded → core-functionality/templates/templates-gallery.spec.ts
- [x] Category tabs — each tab lists exactly the registered templates whose `tags` contain its id (Assistants `assistants`, Classification `classification`, Coding `coding`, Content Generation `content-generation`, Q&A `q-a`, Prompting `chatbots`, RAG `rag`, Agents `agents`), and a tab no visible template is tagged with is not offered → core-functionality/templates/templates-gallery.spec.ts
- [x] Get started tab — the tab the modal opens on shows exactly the featured `template-get-started-card-basic-prompting`, `template-get-started-card-vector-store-rag` and `template-get-started-card-simple-agent` cards and no `template_<slug>` heading, and each card creates its template (`POST /api/v1/flows/` 201 on a new id, flow named after it) → core-functionality/templates/templates-gallery.spec.ts
- [x] Template search — `search-input-template` keeps a template's card for its exact name and leaves zero cards for a string matching no name or description (the match is fuzzy, so the assertion is inclusion or emptiness, never an exact set) → core-functionality/templates/templates-gallery.spec.ts
- [x] Welcome panel quick picks — New Flow opens `flow-builder-welcome-panel`; `flow-builder-welcome-template-simple-agent` and `flow-builder-welcome-template-vector-store-rag` each convert that just-created placeholder **in place** (`PATCH /api/v1/flows/{id}` 200 on the same id, renamed to the template and carrying its graph — they create no flow), and `flow-builder-welcome-browse-more` opens the gallery → core-functionality/templates/templates-gallery.spec.ts
...
- [x] Basic Prompting — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Blog Writer — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Content Aggregator — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Custom Component Generator — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Deep Research Agent — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Document Q&A — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Financial Report Parser — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Hybrid Search RAG — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Image Sentiment Analysis — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Instagram Copywriter — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Knowledge Retrieval — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Market Research — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Meeting Summary — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Memory Chatbot — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Multi Agent Flow — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Portfolio Website Code Generator — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Price Deal Finder — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] SaaS Pricing — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] SEO Keyword Generator — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Sequential Tasks Agents — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Simple Agent — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Social Media Agent — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Text Sentiment Analysis — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Travel Planning Agents — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Twitter Thread Generator — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
- [x] Vector Store RAG — the created flow’s component types, edge count, note count and per-node wiring equal the template’s entry in `GET /api/v1/flows/basic_examples/` (structure, not parameter values) → `core-functionality/templates/templates-instantiate.spec.ts`
...
- [x] A2A Agent component, External mode — pointed at this instance's own card URL, the `agent_card` display renders the card (titled with the target's name, **without** "Requires an API key" while public), a run returns the echoed sentinel, and the target stored it under the A2A server's `<uuid>:<contextId>` session — never the Internal `…:a2a:<id>` one, so the call went over JSON-RPC. The same flow moved into an `auth_type=apikey` project shows "Requires an API key" once `agent_url` is re-pointed (the display does not follow a move on its own), and the owner's key in `api_key` still returns a sentinel; `@regression` for `LE-1845` (`NameError: name 'call_a2a_agent' is not defined`). The loopback block recorded here is gone: `_call_external_agent` validates with the connector policy, which lets a literal loopback host through by default (`connector_ssrf_allow_loopback`), and the self-call completes under `LANGFLOW_WORKERS=1` (measured on `1.13.0.dev12`). A failed card fetch is silent — no error, the display is just absent — so the card's presence is the assertion → `core-functionality/a2a/a2a-client-agent-external.spec.ts`
- [x] A2A Agent used as a Tool — an Agent with the `A2AAgent` wired as a tool (`tool_mode`, Internal mode) and *Requires Approval* on pauses on the approval card (`send_to_agent` + the sentinel in its args) with the target flow **not yet run**; Approve resumes the run, the tool call completes (`tool-status-done`), the target stores the sentinel under `…:a2a:<target id>`, and the caller's persisted `send_to_agent` output carries it. `@regression` for `LE-1963` (`self.user_id is None` → "badly formed hexadecimal UUID string" on tool-approval resume), which lives only in Internal mode and does not reproduce on `1.13.0.dev12`. The model's final wording is not asserted — the same model echoed the sentinel on one run and answered "The message has been sent successfully." on the next. LLM-dependent (`tier: "tool-calling"`) — `--workers=1` + `models.json` → `core-functionality/a2a/a2a-client-agent-as-tool.spec.ts`
...
- [x] Execution order respects data dependency — every consumer's build finishes after all of its producers', asserted on the `/build` direct run stream (finish order + received output, not wall-clock timing) → `api/flows/graph-execution-contract.spec.ts`
- [x] Partial failure on the run stream — when one branch of a multi-branch graph raises, the independent branches still build to completion, the failed node is flagged (`valid: false`) and its descendants never build → `api/flows/graph-execution-contract.spec.ts`
- [x] A branch stopped by its producer (`Component.stop()`, the mechanism If-Else uses) never builds and is reported in `inactivated_vertices`, while the sibling branch still builds → `api/flows/graph-execution-contract.spec.ts`
- [!] A graph with a regular-port cycle must not report a node it never built as completed (**declared failing** — `test.fail()` against a live engine defect, #1896; `POST /api/v2/workflows` `mode=sync` answers `completed` and lists a node downstream of the never-run cycle in `outputs`. The result is name-dependent: a pure cycle whose node id matches the `"webhook"/"chat"` start heuristic runs unbounded instead, which is out of scope here because it would hang the worker) → `api/flows/graph-execution-contract.spec.ts`
- [ ] A node whose upstream produced no value (a producer returning `None`) is skipped rather than run with an empty input — measured on `1.13.0.dev14` the consumer instead **runs** and fails with a pydantic `MessageTextInput` error attributed to itself; recorded in #1896 pending an upstream decision, not pinned
- [x] The canvas refuses a connection that would close a cycle through regular ports, and accepts a connection to the same target port from a node outside the cycle → `flow-functionality/graph-execution-canvas.spec.ts`
- [!] Partial failure on the **canvas** — the failed node is flagged and the branches that completed show as built (**declared failing** — `test.fail()` against the AG-UI defect, #1896: the run stream emits `RUN_ERROR` mid-run and the frontend stops there, so no `node_status_icon` renders and a completed branch's `node_duration` never shows; flips to an unexpected pass the day upstream stops treating a component error as terminal) → `flow-functionality/graph-execution-canvas.spec.ts`
...
- [x] Bundles navigation replaces the Components tree with the vendor-integration tree — clicking `sidebar-nav-bundles` renders the `Bundles` group label and a disclosure per installed vendor bundle, while the `Components` group label goes away → `ui-ux/integration-side-bar.spec.ts` (**promoted from the Wave 9 T2 backlog, #1912.** It measured `0/3 green` for one reason: it asserted `Notion` and `AssemblyAI` by name, and neither is on the tested image — both are `lfx-bundles-shim` families and the Bundles view renders 17 other disclosures instead. The navigation itself always worked. The replacement names — `openai`, `anthropic`, `google` — are lane-guaranteed rather than arbitrary: all three ship as their own installed distributions and the daily rotates its provider across exactly these three (#1185), so a packaging change that removed one would break far more than this spec. That this is still a name assert is recorded in the doc rather than glossed; deriving the expectation from `GET /api/v1/all` was rejected there because it would assert the catalog against itself)
...
- [x] Pan the canvas by dragging the empty pane — a left-button drag from a point `elementFromPoint` confirms is `.react-flow__pane` moves the viewport `translate` by exactly the cursor delta while `scale` stays byte-identical, and the same drag with Shift held (the marquee gesture) moves nothing (negative control) → `flow-functionality/canvas-pan-drag.spec.ts`
...
- [x] A loopback provider base URL is refused, and the message names **every** address the name resolves to (`::1` *and* `127.0.0.1`) plus the `LANGFLOW_SSRF_ALLOWED_HOSTS` escape hatch — both legs, because a message naming only one means the other went unchecked, and `localhost` resolving to `::1` first is the common case → `security/model-provider-base-url-ssrf.spec.ts`
- [x] The cloud-metadata address `169.254.169.254` is refused, with the address named → `security/model-provider-base-url-ssrf.spec.ts`
- [x] A non-`http(s)` scheme is refused naming the scheme (`'file'`) — a different validator branch from the two above, which resolve a host, so a regression could close one and leave the other open → `security/model-provider-base-url-ssrf.spec.ts`
- [x] The **Anthropic** component refuses the same three values through its differently-named field (`base_url` vs OpenAI's `openai_api_base`) — the asymmetry `#14704` existed for, and the reason a one-component spec would have passed the regression it was written for → `security/model-provider-base-url-ssrf.spec.ts`
...
#### 17.6 Credential Secrets Across Graph Edges (1.12)

> `langflow-ai/langflow#14216` (merged 2026-07-22): `Component._get_output_result()` sanitized its return value **in place** before storing it in `Output.value`, and `ComponentVertex._get_result()` prefers that cache for connected **edges** — so a downstream component received the literal mask instead of the real value. Masking in `_build_results()` alone was rejected upstream because downstream would then get the real value without knowing it came from a secret input, so the fix carries runtime-only secret metadata across the edge. **Either half asserted alone is worthless**: masking alone *passes on the defect* (before the fix the value had already been destroyed, so "the secret does not appear" was more true, not less), and delivery alone passes on a build that masks nothing. Distinct from § 17.3, whose flow is deliberately independent **root** nodes with no edges. Spec doc: `docs/security/credential-secret-across-edges.md`.

- [x] The edge delivers the **real** value: a downstream reports `received_len=<len(sentinel)>` and not the mask's ten. The sentinel's length is asserted `!== 10` in the spec, because the mask is `**********` and a ten-character sentinel would make the real value indistinguishable from what a pre-fix build delivered. This is also the control that the credential resolved at all — a length is impossible to produce without it, and a failure to resolve reads `0`, distinguishable from both 22 and 10 → `security/credential-secret-across-edges.spec.ts`
- [x] The **upstream's own** display copy is masked on that same run — the node that put the real value on the edge shows only `**********`, and the sentinel is absent from its whole slice. Simultaneous with the row above by construction: one run, one graph → `security/credential-secret-across-edges.spec.ts`
- [x] A downstream that **re-emits** the secret it legitimately received is masked too — the metadata-propagation half. It read the real value (its sibling proves the edge delivered it), so without the secret metadata riding the edge its display copy would be plaintext: exactly the exposure `_build_results()`-only masking would have left open → `security/credential-secret-across-edges.spec.ts`
- [x] The sentinel appears **nowhere** in the run response, asserted over the whole payload rather than one node's slice, and paired with the mask being present — otherwise "the secret is absent" also passes on a run that produced no output at all → `security/credential-secret-across-edges.spec.ts`
- [ ] Raw secret metadata surviving **vertex serialization** — #14216's fourth claim, tracked in #1600. It reaches the `vertex_build` rows rather than the run response, and whether it is observable through any API needs measuring before it can be specified
- [ ] Multi-hop propagation (`A → B → C`) — one edge is asserted; a single hop is what the root cause describes and what a regression in the `Output.value` cache would break first
...
- [-] **The history is the only record that the deployment declared anything**: once a runtime write supersedes it, `policy-bundle/history` still holds the environment revision **with its `source` and its declared keys** — not merely a revision number, which the OSS sibling already covers. `requireEnvironmentPolicy` reads exactly that to tell a declared instance from an API-written one, so losing it would make every spec in `enterprise/governance/` skip while blaming the reader's start command: a green all-skip on a correctly configured instance (#1010/#1012), arriving through a product change nothing else would notice → `enterprise/governance/declared-policy-provenance.spec.ts`
- [-] And **no current-state surface names the declaration it superseded** — `policy-bundle` and `catalog-policy/components` report the API values only, which is what makes the claim above exact rather than decorative: the history is the sole record. A build that fixes #1559 by surfacing the disagreement turns this red, and that is the correct signal — the spec's premise would have changed → `enterprise/governance/declared-policy-provenance.spec.ts`
...
- [-] The self-service reset refuses the account's **current** password as the new one — `400 You can't use your current password`, the route's only content rule and uncovered until #1558's re-measurement put it in place of the expected-red minimum-length assertion → `enterprise/auth/self-service-password-reset.spec.ts`
...
- [ ] Whether the **self-service** reset should enforce the forced path's minimum — measured that it does **not** (a **one-character** password answers `200`), and #1558's re-measurement found the asymmetry is **declared deliberate** rather than an oversight: EE's `ForcePasswordChangeRequest` comment scopes `min_length=8` to "a security-critical admin/break-glass recovery flow", states it is "not inherited from an existing convention", and records that OSS's reset-password validates nothing beyond "differs from the current password". It is the only password minimum in the Enterprise tree, and a **third** path has none either — the CLI `langflow admin reset-password` refuses only an empty password, which is what makes the declared rationale worth putting to the product rather than asserting: the CLI **is** the break-glass recovery flow it names. Asserted in neither direction, same reasoning as the row below
...
- [-] **Entering edit mode offers one `switch` per model and writes nothing yet** — a screen that wrote on entering an edit mode would be surprising in the worst way and nothing else would notice. (`Edit models` was filed as opening nothing, #1659, on three measurement errors — looking for a dialog, reading a shrinking panel as a collapse, and counting `button` where the control is a `switch`. Closed invalid) → `enterprise/admin-console/model-availability-terms.spec.ts`
- [-] **Enabling a model and saving lands exactly that model in the policy** — `PUT /api/v1/model-availability-policy`, keyed on the model the switch **named** rather than on a position → `enterprise/admin-console/model-availability-terms.spec.ts`
- [-] **Enabling a model does not make it visible while the provider has no credentials** — the row stays `Pending` / `Hidden`. Availability and builder visibility are different axes and the screen does not conflate them; the natural assertion (*the row stops reading Hidden*) is **false**, and asserting it would have manufactured a defect → `enterprise/admin-console/model-availability-terms.spec.ts`
- [-] **`Cancel` discards the edit**, asserted on the request rather than on a state read taken immediately after → `enterprise/admin-console/model-availability-terms.spec.ts`
...
- [x] Two distinct planted sentinels appear in the raw **text** of no client-reachable response — the `201`, the list, `PATCH`, `/health`, `/test`, `/revoke`, `GET /api/v1/integrations` and `GET /api/v1/variables/`. Asserted on the text, not the parsed object: a token nested under an unexpected key survives a key check. `GET /connections/{id}` is **not** in the sweep — that route does not exist, and its SPA catch-all `404` would prove nothing → api/connections/api-connections-secret-boundary.spec.ts
- [x] `has_credentials` is `true` while the create response carries the **exact 16-key** `ConnectionRead` set (and revoke the 17-key one) — asserted as the exact set, never as an absence list for `access_token` / `refresh_token` / `credentials`, which would miss a new field called `token`, `secret` or `envelope` → api/connections/api-connections-secret-boundary.spec.ts
- [x] **A refused body is a response too:** three malformed creates, each carrying both sentinels, answer a redacted `422` with a one-entry `detail` naming its own `type` and `loc` — the langflow#15038 regression (before `1.13.0.dev10` a `422` echoed the offending `input`, i.e. the token) pinned from outside, and at zero write budget since a `422` is refused before the handler → api/connections/api-connections-secret-boundary.spec.ts
- [x] `POST /test` with a `required_scopes` the connection does not hold answers **`200`**, not a refusal, and does **not** name the missing scope: `health` flips to `unhealthy` while `status` stays `ready` and `status_reason` stays `null` — scope coverage describes the request, not the stored credential — re-read from the list rather than re-probed, since `/test` and `/health` are **5 per minute per user** (measured: the 6th answers `429`) and a `VALIDATE` burst is three runs → api/connections/api-connections-secret-boundary.spec.ts
- [x] `status_reason` is `null` on every status a client can reach (`ready` → `expired` via a past `expires_at` + `/health` → `revoked`) → api/connections/api-connections-secret-boundary.spec.ts
- [ ] `status_reason` as `credential-missing` vs `credential-undecryptable` — the distinction that sends a user to re-authorize versus to fix the instance key. **Not black-box reachable:** `status: error` needs the instance key to change under a stored envelope, the envelope row to vanish, or a configured OAuth registration (the nightly answers `registrations: []`). The enum also has five values, not two, and `_record_failed_authorization` writes a reason **without** setting `error`, so "a reason implies error" is not a contract to assert

#### 24.3 Capability manifest and effective policy (#1968)

- [x] Every provider row carries the exact 8-key `IntegrationProviderRead` set and every capability the exact 13-key one, with `risk`, `identity`, `substrate` and `maturity` asserted against the **declared** upstream domain and not the measured one — `risk` has a third value (`destructive`) and `substrate` a third (`mcp`) that nothing ships yet, so pinning what is shipped would redden the first INT-10..12 capability. Shape per row, never a count → api/connections/api-integrations-manifest.spec.ts
- [x] **Cross-check:** each capability's `ext:<provider>:<component_ref>@official` is a key of `GET /api/v1/all` **and** that entry's own `namespaced_id` is the same string, indexed over real categories only (`component_display_names` is a lowercased metadata map, not a category). No upstream test crosses the manifest and the catalog, so a component renamed on one side is a placed node that cannot resolve and nothing sees it. Carries a non-vacuity floor: `component_ref` is nullable upstream, so a loop that skipped nulls would pass having checked nothing → api/connections/api-integrations-manifest.spec.ts
- [x] Capabilities sharing an `auth_profile_id` declare the same `identity` — the observable half of upstream's profile↔identity check, and what makes the `slack.bot.*` / `slack.user.*` split (`bot` against `user_delegated`) asserted without hardcoding that slack ships both → api/connections/api-integrations-manifest.spec.ts
- [x] `enabled` and `connection_count` are derived: the invariant `enabled === (approved && connection_count > 0)` on every row of every read, plus one seeded connection moving `microsoft` from `c0` to `c0 + 1` and back while the `slack` control does not move at all — asserted as a delta against a baseline read inside the test, never as an absolute, since the suite shares one superuser. Removal is re-read from the manifest, never inferred from the `DELETE` status (#1759/#1777/#1807) → api/connections/api-integrations-manifest.spec.ts
- [x] `allowed: true` / `blocked_policy_key: null` on every capability **while** `/policy/effective` reports `unrestricted: true`, an empty `blocked_action_keys`, `managed_externally: false` and `approved_provider_ids` deep-equal to both `loaded_provider_ids` and the manifest's provider ids — the two endpoints asserted as agreeing, so a future policy change cannot be silently one-sided → api/connections/api-integrations-manifest.spec.ts
- [x] Every capability is governable: each `policy_keys` entry parses as `integrations.<provider_id>.<action>` inside its **own** provider's namespace (≥3 non-empty segments, lowercase identifier syntax), and `deployment_contexts` is a non-empty, duplicate-free subset of {hosted, self_managed, desktop, headless}. A key outside the grammar is a manifest bug — governance blocks on these, so the operator could never block that action → api/connections/api-integrations-manifest.spec.ts
...
- [x] A row seeded through the API renders its display name, handle, owner, account — the account line **and** the executing-identity line beneath it (*The signed-in user* / *A bot*, since langflow#15263), each exact, so a subtitle that ignored the connection fails — status badge, **and both** the scope count and the abbreviated scope list (screen-reader text) — a count alone passes on an empty list → core-functionality/integrations/connections-page.spec.ts
- [x] The `Mine` / `Instance` / `Other users` tabs each hold only their own ownership kind — a `user`-owned connection, an `instance`-owned one, and another user's private one, which must stay out of a superuser's `Mine` (the defect langflow#15182 fixed) and whose Owner reads *Other user* (*Shared* until langflow#15263) → core-functionality/integrations/connections-page.spec.ts
...
- [x] All templates renders one card per registered template → `templates-gallery.spec.ts`
- [x] each category tab lists exactly the templates carrying its tag → `templates-gallery.spec.ts`
- [x] Get started shows exactly the three featured cards and opens by default → `templates-gallery.spec.ts`
- [x] the featured card for <featured.name> creates its template → `templates-gallery.spec.ts`
