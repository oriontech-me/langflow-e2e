...
- [-] The edge delivers the **real** value: a downstream reports `received_len=<len(sentinel)>` and not the mask's ten. The sentinel's length is asserted `!== 10` in the spec, because the mask is `**********` and a ten-character sentinel would make the real value indistinguishable from what a pre-fix build delivered. This is also the control that the credential resolved at all — a length is impossible to produce without it, and a failure to resolve reads `0`, distinguishable from both 22 and 10 → `security/credential-secret-across-edges.spec.ts`
- [-] The **upstream's own** display copy is masked on that same run — the node that put the real value on the edge shows only `**********`, and the sentinel is absent from its whole slice. Simultaneous with the row above by construction: one run, one graph → `security/credential-secret-across-edges.spec.ts`
- [-] A downstream that **re-emits** the secret it legitimately received is masked too — the metadata-propagation half. It read the real value (its sibling proves the edge delivered it), so without the secret metadata riding the edge its display copy would be plaintext: exactly the exposure `_build_results()`-only masking would have left open → `security/credential-secret-across-edges.spec.ts`
- [-] The sentinel appears **nowhere** in the run response, asserted over the whole payload rather than one node's slice, and paired with the mask being present — otherwise "the secret is absent" also passes on a run that produced no output at all → `security/credential-secret-across-edges.spec.ts`
