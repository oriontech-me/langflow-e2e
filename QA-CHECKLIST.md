...
- [-] Maximum agent iterations → `core-functionality/llm-agents/agent-max-iterations.spec.ts` (both halves of the causal pair `@stable`; #1264's second pass added the tool-step precondition to Test 1 — see §6.2 for the mechanism and the spec doc's Tags section for the per-provider measurement). **Quarantined 2026-09-23 (#1991):** "agent stops when max iterations is reached" is `test.fixme` without `@stable` for a confirmed product regression, [LE-2728](https://datastax.jira.com/browse/LE-2728). The cap fires, but the limit message is lost whenever the model writes text alongside its tool call. The causal control keeps `@stable`. Lifting the quarantine is a deliverable of #1991.)
