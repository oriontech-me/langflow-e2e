...
- [x] v2 batch requires its trailing slash: without it `POST` answers `405` and `DELETE` answers `422` with `type: "uuid_parsing"`, `loc: ["path","file_id"]`, `input: "batch"` — the body is the evidence the request reached the *single-file* route — **and the file is still listed**, which is the load-bearing half: a refusal that had nonetheless deleted it is the actual hazard → `api/files/api-files-v2-store.spec.ts`
...
> 595 `test()` calls carrying the `@stable` tag, distributed across 226 spec
