...
- [x] `GET /{project_id}/composer-url` returns a URL that resolves — asserted by an MCP handshake against it — and the JSON the copy control puts on the clipboard carries that URL verbatim → `mcp/server/mcp-server-install.spec.ts`
