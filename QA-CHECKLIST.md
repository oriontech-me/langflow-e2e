...
- [ ] Remaining per-tab operator behaviour — audit filters that actually filter, provider add/deactivate, the model blocklist round trip, role assignment. One follow-up each, all gated on the shell above
...
| `enterprise/` — Enterprise-only Surfaces (not scheduled — decision) | 81 | 0 | 64 | 8 | 9 |
