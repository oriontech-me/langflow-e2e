...
- [x] `enabled_providers.provider_status` is keyed by the same names `/providers` returns (set equality, not a count) and every value is a boolean; `enabled_models` is `{provider: {model_id: boolean}}`; `default_model` reads `null` per `model_type`; `model_options/{language,embedding}` answer `200 []` on a keyless instance → `api/models/api-models-catalog.spec.ts`
