...
- [-] Configure and execute flow with Mistral → `model-provider/mistral-provider.spec.ts` (**parked** on the same `lfx-bundles` packaging decision as Groq above, with the same marker rationale and the same standing answer in `docs/component-distribution-policy.md` — re-confirmed on `1.13.0.dev8`/`1.13.0.dev12`, where `mistral` appears only as Bedrock model ids and an Astra Vectorize provider name, never as a component. `MISTRAL_API_KEY` has been a repository secret since 2026-07-09 too, and changes nothing while the pre-flight skips first. See #1039)
