...
| **Status** | **Fixed** by [langflow-ai/langflow#15143](https://github.com/langflow-ai/langflow/pull/15143) — *fix(export): keep global-variable bindings in flow and project exports* (commit `0d8b0be173`, merged 2026-09-17 into `release-1.12.3`, carried into `release-1.13.0` by the merge `c5f42da5aa` of 2026-09-22): the three export call sites now pass the owner's global-variable names to `strip_flow_secrets`, so a bound value naming one of them keeps the name. First nightly carrying it: `1.13.0.dev21`, where the declared-failing Test 4 passed 3/3 on both daily lanes (#2008) and the single-id export, a multi-id ZIP and the project *Download* all keep both bindings with no sentinel (measured 2026-09-23). The flow-version reads (`strip_version_data`) were **not** changed and still null a bound value — outside this report's reach, which is the export |
| **Last re-checked** | 2026-09-16 (before the fix) — **no upstream fix on any watched ref**, and reproduced 5/5 on `1.13.0.dev14`. Evidence in §6 |
