...
|---|---|---|
| **`LANGFLOW_A2A_ENABLED=true`** on the Langflow container | everything except the disabled-state case | **Off everywhere.** Measured on both `dev10` and `dev14`: `GET /api/v1/config` (authed) → `a2a_enabled: false`; `GET /api/v1/a2a/agents` → `404`. `scripts/start-langflow-docker.sh` sets no A2A env at all today. Must be set there, in `scripts/start-langflow-pip.sh`, and in every service container in `daily-stable.yml`, `pr-validation.yml`, `nightly.yml`, `manual.yml`, `weekly-stable.yml` |
