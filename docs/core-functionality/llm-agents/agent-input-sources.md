...
- **Autosave race (root-caused during authoring):** `button_run_agent` builds the
  *persisted* flow, not the live canvas graph. When this test runs after another
  ...
  faster on a cold google provider. What the spec relies on instead is the guard:
  `waitForFlowSaveSettled` before the run removes the race deterministically — the
  same one `agent-system-prompt.spec.ts` uses.
