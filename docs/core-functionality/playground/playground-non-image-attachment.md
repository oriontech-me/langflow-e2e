...
4. Assert `page.getByText(/\.\.\.txt$/).first()` is visible — matches the literal `...{ext}` suffix produced by `formatFileName(name, 10)` whenever the rendered name length exceeds 10 chars (which is true both with and without the server's timestamp prefix)
...
- `src/frontend/src/components/core/playgroundComponent/chat-view/utils/file-preview-display.tsx` — both compact (input) and expanded (sent message) non-image branches; if either renders an `<img>` for non-image files, this test fails (the desired failure mode). The expanded variant call site is `formatFileName(fileInfo.name, 10)` — if the literal `10` constant is changed, the rendered text length changes but the `...{ext}` suffix remains, so the regex still triggers
- `src/frontend/src/components/core/playgroundComponent/chat-view/utils/file-utils.ts` — `formatFileName(name, maxLength)` produces `${baseName.slice(0, maxLength)}...${fileExtension}` whenever `name.length > maxLength` and `baseName.length > 6`; the regex `/\.\.\.txt$/` matches the literal `...{ext}` suffix in that template, independent of the `maxLength` value. If the template is rewritten (e.g. different separator), the test fails loudly
