...
5. `GET /api/v1/projects/` → a project named after the uploaded file (extension
   ...
   | `404` — not visible to this read | the name IS now listed | `LE-2598`'s window, caught **closing between the two diagnostic reads** — row 1 seen a few milliseconds early. The flow readback is issued first (`api-projects-transfer.spec.ts`, the diagnosis array), so a `404` there and a listed project here means the commit landed in between. **Transient.** Two earlier revisions of this cell were wrong in opposite directions: it read *"the reverse divergence: a project holding nothing"*, which the route rules out, and then `UNDECIDED` on the grounds that the project committing says nothing about the flow — circular, since that only holds if divergence is already assumed. `upload_project_flows` runs both writes in **one** `DbSession`, `add` + `flush` with no commit of its own (`projects_files.py:93-94`), and `session_scope` commits once at teardown, so the two rows become visible together. |
