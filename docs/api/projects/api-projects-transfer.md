...
2. Create a flow inside it; download again → `200`, and the first four bytes of
   ...
   **The pair narrows the failure to four cases, and two of them are a verdict.** The
   ...
   rows: it reads the flow **and** the project listing, and carries a row for the one
   direction in which they can diverge — the other direction is ruled out by the route,
   which is argued in that table's row 3.) In the family's vocabulary: `api-flows-batch` and `api-invalid-key`
...
5. `GET /api/v1/projects/` → a project named after the uploaded file (extension
   ...
   | `200` — the row EXISTS | still ABSENT | the flow row and the project row have **diverged** — the import committed one and not the other. Not a window; a data defect, and the asymmetry with row 3 is the read ORDER rather than a different rule: the flow is read first, so a flow already visible and a project still missing afterwards requires visibility to go backwards, which no window produces. The **category** is sound; check the named mechanism before acting on it, because `generate_unique_folder_name` renames a colliding import to `"<name> (1)"`, which produces this same pair with both rows committed and nothing diverged. |
   | `404` — not visible to this read | the name IS now listed | Most likely `LE-2598`'s window caught **closing between the two diagnostic reads** — row 1 seen a few milliseconds early: the flow readback is issued first (`api-projects-transfer.spec.ts`, the diagnosis array), so a `404` there and a listed project here is the commit landing in between. **Transient in that case** — but the same pair is also produced by a flow **removed after** the commit (the cross-worker wipe row 2 of test 1's table names), which is not, so confirm with a later read before calling it transient. What the pair does **rule out** is the reverse divergence — *a project holding nothing* — which is what an earlier revision of this cell claimed: `upload_project_flows` stages the project (`projects_files.py`, `session.add(new_project)` + `flush()`, **207** on `release-1.13.0` and 190 on `main`) and then the flows through `create_flows` (`flows.py`, `add` + one `flush`) on the **same** `DbSession`, neither committing, and `session_scope` commits once at teardown — so an import cannot land one row without the other. A second revision read the pair as `UNDECIDED` on the grounds that the project committing says nothing about the flow; that is circular, since it only holds if divergence is already assumed. |
