...
1. Register a `page.on("response")` listener recording the id of every flow the page creates (201 responses to `POST /api/v1/flows/`) for id-scoped cleanup — this catches both the blank flow and any transient flow the bootstrap "New Flow" entry point creates (no pre-test wipe — see Notes)
2. Call `awaitBootstrapTest`, wait for the network to go idle (lets the home page's own transient-flow sweep finish before we create a flow — see Notes), then click `blank-flow`
...
1. Delete every flow id recorded by `addWebhookComponent` via `deleteFlow` (authenticated with `getAuthToken`), so each test removes exactly the flows it created and nothing else
...
- **Cleanup is id-scoped — never a pre-test wipe.** `addWebhookComponent` used to call `cleanAllFlows` before each test "to avoid 400 'flow must be unique' under parallelism". That wipe deleted flows other parallel workers were actively driving — it was the collider behind the recurring `tool-mode.spec.ts` daily flake (#464: the victim's build request started 404ing `"Flow not found"` on its own flow id). The uniqueness justification no longer holds: the backend auto-suffixes duplicate names (`Untitled document (1)`), verified with three consecutive 201s for the same name on 1.11.0.dev36. Each test now records the id of every flow its page creates (201 responses to `POST /api/v1/flows/`) and `test.afterEach` deletes exactly those ids — `deleteFlow` treats an already-deleted id (404) as success, so transient bootstrap flows the frontend removes on its own are safe to track.
- **Post-bootstrap network-idle wait.** When transient "New Flow" flows exist at home-page mount, the Langflow frontend sweeps them with a batch `DELETE /api/v1/flows/` of its own. If our blank-flow `POST` lands mid-sweep, SQLite raises `database is locked` and the sweep 500s (`Unable to cascade delete flow`, upstream weakness in `delete_multiple_flows` — log-only via the fixture, no test impact, no flow leak since afterEach still deletes tracked ids; observed 2× in 5 file runs). Waiting for network idle after `awaitBootstrapTest` serializes the sweep before our flow creation, removing the contention window. The old pre-test wipe masked this by emptying transients first — the wait replaces that side-effect without the destructive wipe.
