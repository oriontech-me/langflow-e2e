...
6. Select a working OpenAI chat model on the Language Model component via the shared `setupLanguageModelOpenAI` helper. On the 1.11 unified `ModelInput` widget the model dropdown (`model_model`) is present immediately — there is no separate "Setup Provider" button and no per-node API-key field; the provider is configured globally by the "Collect models" step, so the helper only opens `model_model` and picks a model. The dropdown is opened with a dispatched click (`dispatchEvent("click")`), not a hit-tested `.click()`: at zoomed-out scale the bound `api_key` popover renders over the ~10px-tall `model_model` trigger and intercepts a normal click (issue #580). **The dropdown mixes providers and lists the Anthropic default `claude-sonnet-5` first; that default is a reasoning model that rejects the `temperature` param the component sends ("temperature is deprecated for this model" → HTTP 400 → the Playground AI message never resolves). This was the root cause of #722** — the helper now prefers a `gpt-*` (OpenAI) option over the provider-agnostic first-match fallback
...
- Test 3 requires `OPENAI_API_KEY` in the environment; it is skipped gracefully when the key is absent. The OpenAI provider must be configured globally in the instance (the "Collect models" step does this in CI) — the 1.11 `ModelInput` widget reads the global provider config, so selecting a `gpt-*` model has no per-node key to fill. The test selects a working OpenAI model before executing the flow — the template ships defaulting to the Anthropic reasoning model `claude-sonnet-5`, which fails the run (temperature rejected) without this re-selection
...
- **Test 3 daily-#722 hard-failure was a test defect from an upstream widget change, not a product regression (fix restores `@stable`):** the Language Model component moved to the unified `ModelInput` widget (1.11), whose curated dropdown lists only the latest model per provider and defaults to the Anthropic reasoning model `claude-sonnet-5`. None of the helper's `PREFERRED_CHAT_MODELS` (`gpt-4o-mini`, …) appear in that curated list, so the provider-agnostic fallback selected the first option — `claude-sonnet-5` — and the run 400'd with "temperature is deprecated for this model", leaving the Playground AI message on the placeholder-dots state until the 240s timeout (the exact `toContainText(/title/i)` failure in the daily). Reproduced live on `1.11.0.dev38`; selecting `gpt-5.6` instead produces a real aggregated response containing "title". Fix: `setupLanguageModelOpenAI` now includes the current curated OpenAI models and prefers a `gpt-*` option over the first-match fallback. The product itself is healthy — the Loop iterates correctly with a compatible model
- Test 3 re-selects a working model on the Language Model component before running the Playground — the template ships defaulting to `claude-sonnet-5`, causing a build failure if the re-selection step is skipped
