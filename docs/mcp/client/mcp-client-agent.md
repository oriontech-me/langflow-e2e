...
9. Verify last chat message contains `"hello mcp"`

---

## Validation criterion *(required)*

- Agent response contains `"hello mcp"` (confirms the MCP echo tool was called and its result was returned to the user)
...
- Tool call reasoning step visibility in Playground (only the final response is asserted)
...
- Handle testids for the connection step: `handle-mcp tools-shownode-toolset-right` (MCPTools output) and `handle-agent-shownode-tools-left` (Agent tools input).
