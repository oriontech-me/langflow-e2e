...
- `--workers=4` (the default) can intermittently produce a `500 Internal Server Error` on `POST /api/v1/flows/` due to flow-creation contention between workers. The test still passes (the fixture flags it as a warning, not a failure), but `--workers=1` is the deterministic configuration. The CI workflow uses parallel workers so this warning may surface in nightly runs without indicating a real bug.
