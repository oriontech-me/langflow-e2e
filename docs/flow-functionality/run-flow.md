...
- The spec does not load `.env` itself: `playwright.config.ts` loads the working directory's `.env` (the repo root, where the suite is run from) before collection, and CI injects the variables directly. The spec-level `dotenv.config()` it used to carry resolved to `tests/.env`, which does not exist, so it never loaded anything (#2014).
