...
- Weekly triage #364: the UI test failed once on the weekly run (`publish-switch` clicked but stayed `unchecked` at step 10). Root cause was a transient backend overload — the run logged a storm of `500`s on `/api/v1/flows/`, `/api/v1/validate/prompt`, and `/api/v1/build/.../flow`. The `publish-switch` is a controlled Radix switch that only flips after a successful `PATCH access_type=PUBLIC`, so the failed PATCH left it unchecked. Not a UI/build regression: re-validated green (5/5) against the exact failing image digest `sha256:55a586…`. `@stable` restored without changing the test — retry-on-500 was deliberately not added, since a genuine `500` should fail (the fixture's backend-error monitor already flags it).
